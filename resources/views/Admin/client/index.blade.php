...
@section('content')
<div class="container">
  <h4 class="mb-3">Manajemen Clients</h4>
  ...
  <div class="d-flex align-items-center justify-content-between gap-2 flex-wrap mb-3">
    ...
    <form method="GET" action="{{ route('admin.client.index') }}" class="d-flex align-items-center gap-2 w-full">
      <input type="search" name="q" class="form-control" placeholder="Cari link…"
      ...
      @if(($q ?? '') !== '' || ($sort ?? 'desc') !== 'desc')
        <a href="{{ route('admin.client.index') }}" class="btn btn-outline-secondary">Reset</a>
      @endif
    ...
    <table class="table table-bordered table-striped align-middle">
      ...
      <tbody>
        ...
        <tr>
          <td>
            @if($item->gambar)
              <img src="{{ asset('storage/'.$item->gambar) }}" width="120" class="img-thumbnail" alt="logo">
            @else
          ...
          <td>
            ...
            <form action="{{ route('admin.client.destroy', $item->id) }}" method="POST" class="d-inline delete-form">
              @csrf @method('DELETE')
        ...
        @empty
          <tr><td colspan="3" class="text-center text-muted">Belum ada client.</td></tr>
        @endforelse
      </tbody>
    </table>
  </div>
