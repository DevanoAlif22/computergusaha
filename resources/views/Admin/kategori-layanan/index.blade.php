...
@section('content')
<div class="container">
  <h4 class="mb-3">Manajemen Kategori Layanan</h4>
  ...
  <div class="d-flex justify-content-between align-items-center gap-2 flex-wrap mb-3">
    ...
    <form method="GET" action="{{ route('admin.kategori-layanan.index') }}" class="d-flex align-items-center gap-2 ms-auto">
      <input type="search"
             name="q"
             class="form-control"
      ...
      @if(($q ?? request('q')) !== null && ($q ?? request('q')) !== '')
        <a href="{{ route('admin.kategori-layanan.index') }}" class="btn btn-outline-secondary">Reset</a>
      @endif
        ...
        @forelse($kategoris as $kat)
          ...
          <tr>
            ...
            <td>
              ...
              <form action="{{ route('admin.kategori-layanan.destroy', $kat->id) }}"
          ...
          <div class="modal fade" id="editModal{{ $kat->id }}" tabindex="-1" aria-hidden="true">
            <div class="modal-dialog">
              <div class="modal-content">
                <form action="{{ route('admin.kategori-layanan.update', $kat->id) }}" method="POST">
                  @csrf @method('PUT')
        ...
        @empty
          <tr>
            <td colspan="3" class="text-center text-muted">
              @if(($q ?? '') !== '')
                Tidak ada hasil untuk "<b>{{ $q }}</b>". <a href="{{ route('admin.kategori-layanan.index') }}" class="ms-1">Reset</a>
              @else
                Belum ada kategori.
              @endif
            </td>
    ...
    <div class="modal-content">
      <form action="{{ route('admin.kategori-layanan.store') }}" method="POST">
        ...
        <div class="modal-body">
          <div class="mb-3">
            <label class="form-label">Nama</label>
            <input type="text" name="nama" class="form-control" required placeholder="Misal: Web Development">
          </div>
        </div>
        <div class="modal-footer">
